...
- 若资产已有关联图片，前端会把 `@资产名` 改写为 `@imageN`，并把图片作为 `referenceImages` 传给 VideoGenNode
- 若资产没有图片，则回退为 `asset.dna || asset.description` 文本替换
- VideoGenNode 收到解析后的英文提示词、参考图列表和 `duration: 15`
- 用户点击"发送到画布"或"全部发送到画布"后，会把 `sentToCanvas` 写回 `storyboardByEpisode`，刷新后仍可看到已发送状态
