...
- **资产库视觉对齐**：资产库切换为 Studio 背景、紧凑搜索/类型筛选和更稳定的资产网格密度
- **密钥默认值移除**：后端不再使用 `changeme-*` 开发默认密钥，`JWT_SECRET` 与 `ADMIN_SECRET` 必须在 `.env` 中显式配置
- **视觉系统**：首页改为更接近“影棚中控台”的深色玻璃面板、紧凑信息密度和生产状态表达，保留浅色主题
- **交互保持**：新建、重命名、删除、打开项目、资产预览、复制 Prompt、加入全局库等原有功能继续复用
